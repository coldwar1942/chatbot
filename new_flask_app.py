...
#    isConfirm = read_confirm(line_bot_api,  conn, user_id)
 #   if isConfirm == True:
    ...
    if node_data:
        ...
        if isEnd:
            update_user_progress(conn, user_id, node_id, day_step, phase)
            node_data = fetch_user_node_data(conn, user_id)
...
def push_message_with_id():
    ...
    if phase == True:
        ...
        update_phase(line_bot_api, conn, user_id,7,False)
    else:
        status_code = 400
        response_text = "Fail"
    if status_code == 200:
...
def create_user_node(driver,variable_value):
    query = '''
        ...
        phase:false,fetchNext:false,confirm:false,pushTime:16})
    '''
      ...
      #  print(f'isConfirm: {isConfirm}')
       ...
       #     print(f"""showAnser is {showAnswer}""")
        ...
        if isEnd:
            ...
            update_user_progress(conn, user_id, node_id, day_step, phase)
           # update_user_progress(conn, user_id, node_id, day_step, node_step, question_tag,isEnd,count,msg,tk,phase)
            update_phase(line_bot_api, conn, user_id,0,False)
        #if count > 3:
            #phase = False
...
def update_phase(line_bot_api, conn, user_id,count,isConfirm):
    query = f"""
        MATCH (n:user)
        WHERE n.userID = $user_id
        SET n.phase = true
...
def update_user_progress(conn, user_id, node_id, day_step, phase):
...
def fetch_entity_data(conn, node_id, node_step,user_id):
    query = '''
        ...
        RETURN n.name as name, n.name2 as name2, n.photo as photo,n.photo2 as photo2, r.choice as choice,r.name as quickreply, coalesce(n.video, '') as video,n.pic1 as pic1,n.pic2 as pic2,n.pic3 as pic3
    '''
    query2 = '''
        MATCH (n:user)
        WHERE n.userID = $user_id
